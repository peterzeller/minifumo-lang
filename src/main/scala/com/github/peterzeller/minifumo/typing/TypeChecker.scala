      ...
      case ast.Pattern.Ctor(name, args) =>
        env.exports.ctors.get(name) match
          case Some(ctor) =>
            ...
            val typedArgs = args.zipAll(paramTypes, ast.Pattern.Wildcard()(source), Type.Unknown).map { case (arg, tpe) =>
        ...
        calleeType match
          case Type.Fun(params, result) =>
            if params.length != args.length then
              errors += errorAt(callee.source, s"Call expects ${params.length} args, got ${args.length}")
            params.zipAll(args, Type.Unknown, Expr.Var(ErrorSymbol("<missing>", Type.Unknown), Type.Unknown)(callee.source)).foreach {
