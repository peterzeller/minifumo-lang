...
final case class CtorDecl(name: String, fields: List[CtorField], returnType: Option[Expr])(val source: SourceRange, val comment: String)
...
enum Expr:
  ...
  case LetIn(name: String, tpe: Option[Expr], value: Expr, body: Expr)(val source: SourceRange, val comment: String)
